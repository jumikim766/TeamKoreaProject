...
public interface EmailRepository extends JpaRepository<Email, Long> {
    ...
    // 이메일 상세 조회
    // EmailService에서 소유자 검증할 때 email.account.user.userId까지 접근함
    // 그래서 account만 가져오면 user 접근 시 Lazy 로딩 문제로 500이 날 수 있음
    // account와 user를 같이 fetch join해서 상세 조회 실패 방지
    @Query("""
            SELECT e
            FROM Email e
    ...
    Page<Email> findByAccount_User_UserId(Long userId, Pageable pageable);
