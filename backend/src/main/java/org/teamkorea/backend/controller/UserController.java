...
public class UserController {
        ...
        public ResponseEntity<BaseResponse<Void>> sendEmailChangeCode(
                        Authentication authentication,
                        @Valid @RequestBody EmailChangeSendCodeRequestDto requestDto) {
                Long userId = extractUserId(authentication);
                userService.sendEmailChangeCode(userId, requestDto);

                return ResponseEntity.ok(
                                BaseResponse.success("이메일 인증번호가 발송되었습니다."));
        }

        @PostMapping("/me/email/verify-code")
        public ResponseEntity<BaseResponse<Void>> verifyEmailChangeCode(
                        Authentication authentication,
                        @Valid @RequestBody EmailChangeVerifyCodeRequestDto requestDto) {
                Long userId = extractUserId(authentication);
                userService.verifyEmailChangeCode(userId, requestDto);

                return ResponseEntity.ok(
                                BaseResponse.success("이메일 인증이 완료되었습니다."));
        }
